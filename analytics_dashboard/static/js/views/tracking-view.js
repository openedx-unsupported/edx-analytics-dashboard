define(
  ['backbone', 'jquery', 'underscore', 'utils/utils'],
  (Backbone, $, _, Utils) => {
    'use strict';
    ...
    /**
         ...
         * model.
         */
    const TrackingView = Backbone.View.extend({
      ...
      applicationIdSet() {
        const self = this;
        const trackId = self.model.get('segmentApplicationId');
      ...
      /**
             ...
             */
      trackElementEvent(ev) {
        const self = this;
        const trackedElement = ev.target;
        const properties = Utils.getNodeProperties(trackedElement.attributes, 'data-track-', ['data-track-event', 'data-track-triggered']);
        const eventType = $(trackedElement).attr('data-track-event');
        const trackType = $(trackedElement).attr('data-track-type');
        const triggered = $(trackedElement).attr('data-track-triggered');
      ...
      /**
             ...
             */
      transformPropertiesFromHTMLAttributes(props) {
        const properties = props;
        const targetNameParts = [];
        const parts = ['scope', 'lens', 'report', 'depth'];
        let partVal;

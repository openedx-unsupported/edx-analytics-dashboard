...
define((require) => {
  ...
  const ArrayFieldFilter = require('course-list/common/filters/array-field-filter');
  ...
  const CourseListCollection = ListCollection.extend({
    ...
    /**
         ...
         */
    constructFilter() {
      const activeFilterFields = this.getActiveFilterFields();
      const filters = [];
    ...
    setSearchString(searchString, matcher) {
    ...
    getFilterValues(filterType) {
      ...
      this.programsCollection.each((program) => {

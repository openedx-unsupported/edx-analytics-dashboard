define((require) => {
  ...
  const PagingCollection = require('edx-ui-toolkit/src/js/pagination/paging-collection');
  ...
  const ListCollection = PagingCollection.extend({
    ...
    getQueryString() {
      const params = this.getActiveFilterFields(true);
      const orderedParams = [];
    ...
    clearFilter(filterKey, filterValue) {
      const removedFilter = {};
      const currentValues = this.getFilterFieldValue(filterKey);
    ...
    clearAllFilters() {
      const originalFilters = this.getActiveFilterFields(true);
    ...
    /**
         ...
         */
    getFilterValueDisplayName(filterKey, filterValue) {
      const { filterNameToDisplay } = this;
    ...
    /**
         ...
         *     user
         */
    registerFilterableArrayField(fieldName, displayName) {
